...
<header class="site-header" role="banner">
    ...
    <button type="button" class="nav-toggle" id="nav-toggle" aria-expanded="false" aria-controls="primary-menu" aria-label="Открыть меню">
        <span class="nav-toggle-bar" aria-hidden="true"></span>
        <span class="nav-toggle-bar" aria-hidden="true"></span>
        <span class="nav-toggle-bar" aria-hidden="true"></span>
    </button>
    <div class="site-menu-sheet" id="primary-menu">
        ...
        <div class="header-actions">
            <?php if (!empty($user['is_auth'])): ?>
                <span class="btn btn-sm header-user" title="Вы вошли как <?= htmlspecialchars((string)($user['nick'] ?? ''), ENT_QUOTES) ?>"><?= htmlspecialchars((string)($user['nick'] ?? 'Пользователь'), ENT_QUOTES) ?></span>
            ...
            <?php endif; ?>
            <noscript>
                <div class="header-noscript-note">JavaScript отключён — сайт остаётся доступным.</div>
            </noscript>
        </div>
    </div>
</header>
